...
class WishList(BaseModel):
    user = models.OneToOneField(CustomUser, on_delete=models.CASCADE, related_name='wishlist_user')
    ...
    def __str__(self):
        return f'ID: {self.id} - WishList - User: {self.user} - Count: {self.count}'


class WishListItem(BaseModel):
    wishlist = models.ForeignKey(WishList, on_delete=models.CASCADE, related_name='wishlistitem_wishlist')
    product = models.ForeignKey(Product, on_delete=models.CASCADE, related_name='wishlistitem_product')

    class Meta:
        unique_together = ('wishlist', 'product')

    def __str__(self):
        return f'ID: {self.id} - Wishlist: {self.wishlist} - Product: {self.product}'


class Cart(BaseModel):
    user = models.OneToOneField(CustomUser, on_delete=models.CASCADE, related_name='cart_user')

    @property
    def count(self):
        return self.cartitem_cart.count()
    ...
    def __str__(self):
        return f'ID: {self.id} - Cart - User: {self.user} - Count: {self.count} - Sub Total: {self.cart_sub_total_price} - Total: {self.cart_total_price}'
...
class CartItem(BaseModel):
    ...
    def __str__(self):
        return f'ID: {self.id} - Cart: {self.cart} - Product: {self.product} - Quantity: {self.quantity} - Total Price: {self.total_price}'
...
class Order(BaseModel):
    ...
    def __str__(self):
        return f'ID: {self.id} - User: {self.user} - Status: {self.status}'
...
class OrderItem(BaseModel):
    ...
    def __str__(self):
        return f'ID: {self.id} - Order: {self.order} - Product: {self.product} - Quantity: {self.quantity} - Price: {self.price}'
